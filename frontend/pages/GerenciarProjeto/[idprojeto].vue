...
async function fetchTasks() {
  ...
  try {
    const { data, error } = await useFetch(`http://${process.env.VUE_APP_API_URL}:8081/projetos/${project.value.id}/tarefas`);
...
onMounted(async () => {
  try {
    const { data, error } = await useFetch(`http://${process.env.VUE_APP_API_URL}:8081/projetos/${id.value}`);
    if (error.value) {
